...
# -----------------------------
# Simulovaný model pre report (bez zásahu DB)
# -----------------------------
class Record:
...
# -----------------------------
# Route /report pre zobrazenie farebných dní
# -----------------------------
...
def report():
    ...
    # simulované záznamy (v reálnom projekte nahradiť dátami z DB)
    records = []
    current = start_date
    ...
    # sviatky zo Slovenska
    holidays = get_slovak_holidays(start_date.year)

    # počítanie hodín podľa typu dňa
    normal_hours = 0
    weekend_hours = 0
    holiday_hours = 0
    total_hours = 0
    ...
    for rec in records:
        ...
        if rec.date in holidays:
            holiday_hours += hours
        elif rec.date.weekday() >= 5:  # sobota alebo nedeľa
            weekend_hours += hours
        else:
            normal_hours += hours
